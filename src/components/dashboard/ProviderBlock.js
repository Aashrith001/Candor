...
import { Button,Tag,Icon, Alert } from 'rsuite';
...
const ProviderBlock = () => {
  ...
  const updateIsConected = (providerId,value)=>{
    setIsConnected(p=>{
     return {
       ...p,
       [providerId] : value,
  ...
  const unlink = async(providerId) =>{
    try{
      ...
      updateIsConected(providerId,false);
  ...
  const link = async (provider)=>{
      try{
        ...
        updateIsConected(provider.providerId,true);
      }catch(err){
        Alert.error(err.message,4000);
      }
  }
